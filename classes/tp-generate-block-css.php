...
class Tp_Generate_Blocks_Css {
	...
	public function generate_dynamic_css( $post_id = ''){
		...
		foreach ($parse_blocks as $block) {
			$this->parse_block_settings( $block );
		}
	...
	public function parse_block_settings( $block = []){
		$settings = [];
		if(!empty($block)){
			$context = array();
			...
			if ( ! is_null( $wpblock->block_type ) ) {
				...
				foreach ( $block_attribute as $attribute_name => $schema ) {
					...
					if ( isset( $schema['default'] ) && !isset($schema['repeaterField'])) {
			...
			if(!empty($attributes)){
				...
				if ( !empty( $block['innerBlocks'] ) ) {
					foreach ( $block['innerBlocks'] as $inner_block ) {
						self::$all_attributes = $this->parse_block_settings($inner_block);
					}
				}
			}
