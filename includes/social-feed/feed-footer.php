...
if ( 'Facebook' === $select_feed ) {
	if ( isset( $fblikes ) ) {
		...
		if ( ! empty( $show_footer_in ) && 'style-2' === $style ) {
			echo esc_html__( 'Like ', 'the-plus-addons-for-block-editor' );
		} else {
			echo '<img src="' . esc_url( $like_img ) . '" alt="' . esc_attr__( 'Like', 'the-plus-addons-for-block-editor' ) . '"/>';
			echo '<img src="' . esc_url( $reaction_img ) . '" alt="' . esc_attr__( 'Reaction', 'the-plus-addons-for-block-editor' ) . '"/>';
		}
