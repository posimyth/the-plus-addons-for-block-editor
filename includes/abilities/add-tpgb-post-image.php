<?php
/**
 * Ability: Add Nexter Blocks Post Image (tpgb/tp-post-image) to a Gutenberg page.
 *
...
wp_register_ability(
	'nexter-blocks/add-tpgb-post-image',
	array(
		'label'               => __( 'Add Nexter Blocks Post Image', 'the-plus-addons-for-block-editor' ),
		'description'         => __( 'Adds the Nexter Blocks Post Image block (tpgb/tp-post-image) — automatically displays the current post\'s featured image in default mode (inline image) or background mode (applies to section/body/container). Supports image size, alignment, max width, background positioning, border/shadow styling, lightbox popup. Use in post templates. This is a dynamic block.', 'the-plus-addons-for-block-editor' ),
		...
		'input_schema'        => array(
			...
			'properties'           => array(
				...
				'imageMode'           => array(
					'type'        => 'string',
					'enum'        => array( 'default', 'background' ),
					'description' => '"default" = inline image; "background" = apply as background.',
					'default'     => 'default',
				),
				'backgroundLocation'  => array(
					'type'        => 'string',
					'enum'        => array( 'section', 'body', 'container' ),
					'description' => 'Where to apply the background (when imageMode is "background").',
					'default'     => 'section',
				),
				...
				'bgPosition'          => array(
					...
					'description' => 'CSS background-position e.g. "center center", "top left".',
				),
				'bgAttachment'        => array(
					'type'    => 'string',
					'enum'    => array( 'scroll', 'fixed', 'local' ),
					'default' => 'scroll',
				),
				'bgRepeat'            => array(
					'type'    => 'string',
				...
				'bgOverlayColor'      => array(
					'type'        => 'string',
					'default'     => '',
					'description' => 'Overlay colour (normal).',
				),
				'bgOverlayHoverColor' => array(
					'type'        => 'string',
					'default'     => '',
					'description' => 'Overlay colour (hover).',
				),
		...
		'execute_callback'    => 'tpgb_mcp_add_post_image_ability',
		'permission_callback' => 'tpgb_mcp_add_post_image_permission',
		'meta'                => array(
			'show_in_rest' => true,
			'mcp'          => array(
				'public' => true,
...
/**
 * Permission callback for the add-post-image ability.
 *
 * @param array|null $input Ability input arguments.
 * @return bool True when the current user may insert the block.
 */
function tpgb_mcp_add_post_image_permission( ?array $input = null ): bool {
 ...
 */
function tpgb_mcp_pimg_spacing( array $v ): array {
	return array(
		'md'   => array(
 ...
 */
function tpgb_mcp_pimg_border( array $b ): array {
	$w = $b['width'] ?? array();
	return array(
		'openBorder' => 1,
 ...
 */
function tpgb_mcp_pimg_radius( array $r ): array {
	return array(
		'md'   => array(
 ...
 */
function tpgb_mcp_pimg_bshadow( array $s ): array {
	return array(
		'openShadow' => true,
		'inset'      => $s['inset'] ?? 0,
 ...
 */
function tpgb_mcp_pimg_bg( string $color ): array {
	return array(
		'openBg'         => 1,
		'bgType'         => 'color',
		'videoSource'    => 'local',
 ...
 */
function tpgb_mcp_pimg_needs_wrapper( array $attrs ): bool {
	$keys = array(
		'globalMargin',
		'globalPadding',
		'globalBg',
...
/**
 * Execute callback: insert a tpgb/tp-post-image block into a post.
 *
 * @param array $input Ability input arguments.
 * @return array|WP_Error Ability result or error on failure.
 */
function tpgb_mcp_add_post_image_ability( array $input ) {

	if ( ! defined( 'TPGB_VERSION' ) && ! defined( 'TPGBP_VERSION' ) ) {
		return new WP_Error( 'nexter_blocks_missing', __( 'Nexter Blocks must be active.', 'the-plus-addons-for-block-editor' ) );
	}

	$block_name = 'tpgb/tp-post-image';
	if ( ! tpgb_mcp_has_registered_block( $block_name ) ) {
		return new WP_Error( 'block_missing', __( 'tpgb/tp-post-image is not registered.', 'the-plus-addons-for-block-editor' ) );
	}
	...
	/* ── Display mode ─────────────────────────────────────────────────── */
	if ( ! empty( $input['imageMode'] ) && 'default' !== $input['imageMode'] ) {
		$attrs['imageType'] = sanitize_key( $input['imageMode'] ); }
	if ( ! empty( $input['backgroundLocation'] ) && 'section' !== $input['backgroundLocation'] ) {
		$attrs['bgLocation'] = sanitize_key( $input['backgroundLocation'] ); }

	/* ── Image settings ───────────────────────────────────────────────── */
	if ( ! empty( $input['imageSize'] ) && 'full' !== $input['imageSize'] ) {
		$attrs['imageSize'] = sanitize_key( $input['imageSize'] ); }
	if ( ! empty( $input['alignment'] ) ) {
		$attrs['imageAlign'] = array( 'md' => sanitize_key( $input['alignment'] ) );
	}
	...
	/* ── Background settings ──────────────────────────────────────────── */
	if ( ! empty( $input['bgPosition'] ) && 'center center' !== $input['bgPosition'] ) {
		$attrs['bgPosition'] = sanitize_text_field( $input['bgPosition'] ); }
	if ( ! empty( $input['bgAttachment'] ) && 'scroll' !== $input['bgAttachment'] ) {
		$attrs['bgAttachment'] = sanitize_key( $input['bgAttachment'] ); }
	if ( ! empty( $input['bgRepeat'] ) && 'no-repeat' !== $input['bgRepeat'] ) {
		$attrs['bgRepeat'] = sanitize_key( $input['bgRepeat'] ); }
	if ( ! empty( $input['bgSize'] ) && 'cover' !== $input['bgSize'] ) {
		$attrs['bgSize'] = sanitize_key( $input['bgSize'] ); }

	/* ── Background overlay ───────────────────────────────────────────── */
	if ( ! empty( $input['bgOverlayColor'] ) ) {
		$attrs['postimgBg'] = tpgb_mcp_pimg_bg( $input['bgOverlayColor'] ); }
	if ( ! empty( $input['bgOverlayHoverColor'] ) ) {
		$attrs['postimgHvrBg'] = tpgb_mcp_pimg_bg( $input['bgOverlayHoverColor'] ); }

	/* ── Border ───────────────────────────────────────────────────────── */
	if ( ! empty( $input['border'] ) ) {
		$attrs['postimgbor'] = tpgb_mcp_pimg_border( $input['border'] ); }
	if ( ! empty( $input['borderHover'] ) ) {
		$attrs['postimgHvrbor'] = tpgb_mcp_pimg_border( $input['borderHover'] ); }
	if ( ! empty( $input['borderRadius'] ) ) {
		$attrs['postimgbRad'] = tpgb_mcp_pimg_radius( $input['borderRadius'] ); }
	if ( ! empty( $input['borderRadiusHover'] ) ) {
		$attrs['postimgbhvrRad'] = tpgb_mcp_pimg_radius( $input['borderRadiusHover'] ); }

	/* ── Shadow ───────────────────────────────────────────────────────── */
	if ( ! empty( $input['enableShadow'] ) ) {
		$attrs['postimgBshad'] = array(
			'openShadow' => true,
			'inset'      => 0,
	...
	if ( ! empty( $input['globalMargin'] ) ) {
		$attrs['globalMargin'] = tpgb_mcp_pimg_spacing( $input['globalMargin'] );  }
	if ( ! empty( $input['globalPadding'] ) ) {
		$attrs['globalPadding'] = tpgb_mcp_pimg_spacing( $input['globalPadding'] ); }
	if ( ! empty( $input['globalBgColor'] ) ) {
		$attrs['globalBg'] = tpgb_mcp_pimg_bg( $input['globalBgColor'] );      }
	if ( ! empty( $input['globalBgHoverColor'] ) ) {
		$attrs['globalBgHover'] = tpgb_mcp_pimg_bg( $input['globalBgHoverColor'] ); }
	if ( ! empty( $input['globalBorder'] ) ) {
		$attrs['globalBorder'] = tpgb_mcp_pimg_border( $input['globalBorder'] );       }
	if ( ! empty( $input['globalBorderHover'] ) ) {
		$attrs['globalBorderHover'] = tpgb_mcp_pimg_border( $input['globalBorderHover'] );  }
	if ( ! empty( $input['globalBRadius'] ) ) {
		$attrs['globalBRadius'] = tpgb_mcp_pimg_radius( $input['globalBRadius'] );      }
	if ( ! empty( $input['globalBRadiusHover'] ) ) {
		$attrs['globalBRadiusHover'] = tpgb_mcp_pimg_radius( $input['globalBRadiusHover'] ); }
	if ( ! empty( $input['globalBShadow'] ) ) {
		$attrs['globalBShadow'] = tpgb_mcp_pimg_bshadow( $input['globalBShadow'] );      }
	if ( ! empty( $input['globalBShadowHover'] ) ) {
		$attrs['globalBShadowHover'] = tpgb_mcp_pimg_bshadow( $input['globalBShadowHover'] ); }
	...
	/* ── tpgbDisrule ──────────────────────────────────────────────────── */
	if ( tpgb_mcp_pimg_needs_wrapper( $attrs ) ) {
